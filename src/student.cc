...
void viewStudentName(const string &name, const vector<vector<string>> &data) {
    system("cls");
    cout << "\n\n\n\n" << endl;
...
void changePassword(const string &name, vector<vector<string>>& data) {
    system("cls");
    string newPassword;
    cout << "\n\n\n\t\t\t\tEnter new password: ";
    if(cin.peek() == '\n') cin.ignore();
...
void manifestStudentGUI(const string &studentName) {
    ...
    {
        case 1:
            viewStudentName(studentName, data);
            break;
        case 2:
            // Change password function to be implemented
            changePassword(studentName, data);
            break;
        case 3:
            changeJob(studentName, data);
            break;
        default:
            break;
    }
...
void studentLogin_account() {
    system("cls");
    string studentName, password, truePassword;
    ...
    auto it = find_if(data.begin(), data.end(),
                      [&](const vector<string>& row) { return row[0] == studentName; });
    if (it != data.end()) {
    ...
    if (password == truePassword) {  // Example validation
        ...
        studentLogin_account();  // Recursive call on wrong password
    }
}

void changeJob(const string &name, vector<vector<string>>& data) {
    system("cls");
    string newJob;
    cout << "\n\n\n\t\t\t\tEnter new job: ";
    if (cin.peek() == '\n') cin.ignore();
